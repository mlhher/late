...
func withTempCWDTargetEdit(t *testing.T, fn func(tmpDir string)) {
	t.Helper()
...
func TestTargetEditTool(t *testing.T) {
	withTempCWDTargetEdit(t, func(tmpDir string) {
		file1 := "test.txt"
		content := "line 1\nline 2\nline 3\n"
		filePath := filepath.ToSlash(filepath.Join(tmpDir, file1))
		...
		t.Run("unsafe path rejected", func(t *testing.T) {
			args := json.RawMessage(`{
				"file": "/etc/passwd",
				"search": "root",
				"replace": "root"
			...
			if !strings.Contains(err.Error(), "outside the allowed directory") {
				t.Errorf("expected safe-path error, got: %v", err)
			}
		})
	})
}
